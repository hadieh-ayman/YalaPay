...
async function addPayment(e) {
    ...
    if (paymentSelect.value == "Cheque") {
        ...
        const cheque = {
            ...
            chequeImageUri: data.chequeImageUri,
        };

        if (isEdit) {
            await repo.updatePayment(payment);
            await repo.updateCheque(cheque, oldChequeNo);
            isEdit = false;
        } else {
            const payments = await repo.getPayments();
            payment.paymentId = payments.length + 1;
            await repo.addPayment(payment);
            await repo.addCheque(cheque);
        }
    } else {
...
async function searchPayments(e) {
    ...
    const payment = await repo.getPayment(parseInt(searchInput.paymentId));
    const sessionInvoice = sessionStorage.getItem("invoiceNo");
...
async function deletePayment(paymentID) {
    if (paymentSelect.value == "Cheque") {
        ...
        await repo.deleteCheque(payment.chequeNo);
    }
    await repo.deletePayment(parseInt(paymentID));
    await showPaymentData();
}

async function updatePayment(paymentID) {
    isEdit = true;
    const payment = await repo.getPayment(parseInt(paymentID));
    ...
    showMoreDetails();
    if (paymentSelect.value == "Cheque") {
        const cheque = await repo.getCheque(parseInt(payment.chequeNo));
...
async function showInvoiceInfo() {
    const sessionInvoice = sessionStorage.getItem("invoiceNo");
    const invoice = await repo.getInvoice(parseInt(sessionInvoice));
